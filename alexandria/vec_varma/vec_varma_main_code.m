function [model] = vec_varma_main_code(user_inputs)
    ...
    if conditional_forecast && conditional_forecast_type == 1
        model.conditional_forecast(forecast_periods, conditional_forecast_credibility, condition_table, [], 1, Z_p);
    elseif conditional_forecast && conditional_forecast_type == 2
    ...
    res = Results(model, 'complementary_information', results_information);
    ...
    if create_graphics
        ...
        grp = Graphics(model, 'complementary_information', graphics_information, ...
