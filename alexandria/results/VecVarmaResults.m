classdef VecVarmaResults < handle
    ...
    methods (Access = protected, Hidden = true)
        ...
        function complete_vec_varma_information(self)
        ...
        function add_vec_varma_tab_2_inputs(self)
            % initiate lines
            lines = string([]);
            % header for tab 2
            ...
            % VEC hyperparameters
            if model_type == 1
                lags = num2str(self.model.lags);
                lines = [lines;'p (lags): ' lags]; 
                ...
                lines = [lines;'prior: ' prior_name];
                correction_type = self.model.error_correction_type;
                if correction_type == 1
                    correction_name = 'general';
                elseif correction_type == 2
                    correction_name = 'reduced-rank'; 
                end
                lines = [lines;'correction type: ' correction_name];
                max_cointegration_rank = num2str(self.model.max_cointegration_rank);
                lines = [lines;'r (max cointegration rank): ' max_cointegration_rank];
            ...
            elseif model_type == 2
                ...
                lines = [lines;'delta (AR coefficients): ' ar_coefficients];
                pi1 = num2str(self.model.pi1);
                lines = [lines;'pi1 (overall tightness): ' pi1];
                pi2 = num2str(self.model.pi2);
                lines = [lines;'pi2 (cross-variable shrinkage): ' pi2];
                ...
                pi4 = num2str(self.model.pi4);
                lines = [lines;'pi4 (exogenous slackness): ' pi4];
                residual_lags = num2str(self.model.residual_lags);
                lines = [lines;'q (residual lags): ' residual_lags]; 
                lambda1 = num2str(self.model.lambda1);
                lines = [lines;'lambda1 (overall tightness): ' lambda1];
                lambda2 = num2str(self.model.lambda2);
                lines = [lines;'lambda2 (cross-variable shrinkage): ' lambda2];
                lambda3 = num2str(self.model.lambda3);
                lines = [lines;'lambda3 (lag decay): ' lambda3];
            end
            lines = [lines;' '];
            lines = [lines;' '];
        ...
        function add_vec_varma_tab_3_inputs(self)
            % initiate lines
            lines = string([]);
            % header for tab 1
        ...
        function make_vec_varma_summary(self)
            % initiate string list
            self.estimation_summary = string([]);
            % add model header
            self.add_vec_varma_header();
            % add estimation header
            self.add_vec_varma_estimation_header();
            % make list of regressors
            self.vec_varma_regressors_and_index();
            % loop over equations
            for i=1:self.model.n
                % add coefficient summary
                self.add_vec_varma_coefficient_summary(i);
                % add extension summary
                self.add_extension_summary(i);
                % residual and shock variance
                self.add_vec_varma_shock_variance_summary(i);
                % in-sample fit criteria
                self.add_vec_varma_insample_evaluation(i);
            end
            % cointegration relations
            self.add_cointegration_summary();
            % residual variance-covariance matrix
            self.add_vec_varma_residual_matrix_summary();
            % structural shocks variance-covariance matrix
            self.add_vec_varma_shock_matrix_summary();
            % structural identification matrix
            self.add_vec_varma_structural_identification_matrix_summary();
            % add forecast evaluation criteria, if relevant
            self.add_vec_varma_forecast_evaluation();
        end


        function make_vec_varma_application_summary(self)
            % in-sample fit measures
            self.make_vec_varma_insample_fit_summary();
            % forecasts
            self.make_vec_varma_forecast_summary();        
            % conditional forecasts
            self.make_vec_varma_conditional_forecast_summary();  
            % impulse response function
            self.make_vec_varma_irf_summary();        
            % forecast error variance decomposition
            self.make_vec_varma_fevd_summary();    
            % historical decomposition
            self.make_vec_varma_hd_summary();
        end


        function save_vec_varma_application(self, path)
            % save in-sample fit
            self.save_vec_varma_insample_fit_summary(path);
            % save forecasts
            self.save_vec_varma_forecast_summary(path);   
            % save conditional forecasts
            self.save_vec_varma_conditional_forecast_summary(path);      
            % save impulse response function
            self.save_vec_varma_irf_summary(path);
            % save forecast error variance decomposition
            self.save_vec_varma_fevd_summary(path);
            % save historical decomposition
            self.save_vec_varma_hd_summary(path);
        end


        function add_vec_varma_header(self)
        ...
        function add_vec_varma_estimation_header(self)
            % initiate lines
            lines = string([]);
            ...
            if model_type == 1
                ...
                left_element = sprintf(['Prior:' sprintf('%+32s', prior_name)]);
                right_element = sprintf(['Correction type:' sprintf('%+22s', correction_name)]);
                lines = [lines; [left_element '    ' right_element]];
            elseif model_type == 2
                % fourth row: residual lags
                lags = num2str(self.model.residual_lags);
                left_element = '                                      ';
                right_element = sprintf(['Residual lags:' sprintf('%+24s', lags)]);
                lines = [lines; [left_element '    ' right_element]];
            end
        ...
        function vec_varma_regressors_and_index(self)
            model_type = self.complementary_information.model_type;
            ...
            self.ext_regressors = regressors;
            self.ext_coefficient_index = coefficient_index;
        end


        function add_vec_varma_coefficient_summary(self, i)
            model_type = self.complementary_information.model_type;
            if model_type == 1
                self.add_vec_coefficient_summary(i);
            elseif model_type == 2
                self.add_varma_coefficient_summary(i);
            end
        end


        function add_vec_coefficient_summary(self, i)
            lines = string([]);
            ...
            lines = [lines;cu.string_line('VEC autoregressive coefficients phi:')];
            coefficient_index = self.ext_coefficient_index;
            regressors = self.ext_regressors;
            for j=1:numel(regressors)
                if j > self.model.m
                    regressor = ['D ' char(regressors(j))];
                else
                    regressor = char(regressors(j));
                end
                index = coefficient_index(j);
                coefficient = self.model.Phi_estimates(index,i,1);
                standard_deviation = self.model.Phi_estimates(index,i,4);
                lower_bound = self.model.Phi_estimates(index,i,2);
                upper_bound = self.model.Phi_estimates(index,i,3);
        ...
        function add_varma_coefficient_summary(self, i)
            lines = string([]);
            endogenous_variables = self.complementary_information.endogenous_variables;
            credibility_level = self.model.credibility_level;
            lines = [lines;cu.equation_header(['Equation: ' char(endogenous_variables(i))])];
            lines = [lines;cu.coefficient_header(credibility_level)];
            lines = [lines;cu.string_line('VARMA autoregressive coefficients beta:')];
            coefficient_index = self.ext_coefficient_index;
            regressors = self.ext_regressors;
            for j=1:numel(regressors)
                regressor = char(regressors(j));
        ...
        function add_extension_summary(self, i)
            model_type = self.complementary_information.model_type;
            if model_type == 1
                error_correction_type = self.model.error_correction_type;
                if error_correction_type == 1
                    self.add_correction_summary(i);
                elseif error_correction_type == 2
                    self.add_loadings_summary(i);
                end
            elseif model_type == 2
                self.add_residual_summary(i);
            end
        end


        function add_correction_summary(self, i)
            lines = string([]);
            endogenous_variables = self.complementary_information.endogenous_variables;
            lines = [lines;cu.string_line(['VEC error correction coefficients xi:'])];      
            for j=1:self.model.n
                regressor = [char(endogenous_variables(j)) ' (-1)'];
                coefficient = self.model.Xi_estimates(i,j,1);
                standard_deviation = self.model.Xi_estimates(i,j,4);
                lower_bound = self.model.Xi_estimates(i,j,2);
                upper_bound = self.model.Xi_estimates(i,j,3);
                lines = [lines;cu.string_line(cu.parameter_estimate_line(regressor,...
                         coefficient, standard_deviation, lower_bound, upper_bound))];
            end
            lines = [lines;cu.hyphen_dashed_line()];
        ...
        function add_loadings_summary(self, i)
            lines = string([]);
            lines = [lines;cu.string_line(['VEC cointegration loadings lambda:'])];  
            for j=1:self.model.r
                regressor = ['coint. rel. ' num2str(j)];
                coefficient = self.model.Lamda_estimates(i,j,1);
                standard_deviation = self.model.Lamda_estimates(i,j,4);
                lower_bound = self.model.Lamda_estimates(i,j,2);
                upper_bound = self.model.Lamda_estimates(i,j,3);
                lines = [lines;cu.string_line(cu.parameter_estimate_line(regressor,...
                         coefficient, standard_deviation, lower_bound, upper_bound))];
            end
            lines = [lines;cu.hyphen_dashed_line()];
        ...
        function add_residual_summary(self, i)
            endogenous_variables = self.complementary_information.endogenous_variables;
            n = self.model.n;
            q = self.model.q;
            ...
            lines = string([]);
            lines = [lines;cu.string_line(['VARMA lagged residual coefficients kappa:'])];
            for j=1:numel(regressors)
                regressor = ['res ' char(regressors(j))];
                index = coefficient_index(j);
                coefficient = self.model.kappa_estimates(index,i,1);
                standard_deviation = self.model.kappa_estimates(index,i,4);
                lower_bound = self.model.kappa_estimates(index,i,2);
                upper_bound = self.model.kappa_estimates(index,i,3);
                lines = [lines;cu.string_line(cu.parameter_estimate_line(regressor,...
                         coefficient, standard_deviation, lower_bound, upper_bound))];
            end
            lines = [lines;cu.hyphen_dashed_line()];
        ...
        function add_vec_varma_shock_variance_summary(self, i)
            lines = string([]);
        ...
        function add_vec_varma_insample_evaluation(self, i)
            lines = string([]);
        ...
        function add_cointegration_summary(self)
            model_type = self.complementary_information.model_type;
            if model_type == 1
                error_correction_type = self.model.error_correction_type;
                if error_correction_type == 2
                    ...
                    r = self.model.r;
                    lines = string([]);
                    lines = [lines;cu.equation_header('Cointegration relations Kappa')];
                    lines = [lines;cu.coefficient_header(credibility_level)];
                    for i=1:r
                        if i>1
                            lines = [lines;cu.hyphen_dashed_line()];
                        end
                        lines = [lines;cu.string_line(['Cointegration relation ' num2str(i) ':'])];
                        for j=1:n
                            regressor = char(endogenous_variables(j));
                            coefficient = K(j,i,1);
                            standard_deviation = K(j,i,4);
                            lower_bound = K(j,i,2);
                            upper_bound = K(j,i,3);
                            lines = [lines;cu.string_line(cu.parameter_estimate_line(regressor,...
                                     coefficient, standard_deviation, lower_bound, upper_bound))];
        ...
        function add_vec_varma_residual_matrix_summary(self)
        ...
        function add_vec_varma_shock_matrix_summary(self)
        ...
        function add_vec_varma_structural_identification_matrix_summary(self)
            lines = string([]);
        ...
        function add_vec_varma_forecast_evaluation(self)
            lines = string([]);
        ...
        function make_vec_varma_insample_fit_summary(self)
        ...
        function make_vec_varma_forecast_summary(self)
        ...
        function make_vec_varma_conditional_forecast_summary(self)
        ...
        function make_vec_varma_irf_summary(self)
            endogenous_variables = self.complementary_information.endogenous_variables;
            n = self.model.n;
        ...
        function make_vec_varma_fevd_summary(self)
        ...
        function make_vec_varma_hd_summary(self)
        ...
        function save_vec_varma_insample_fit_summary(self, path)
        ...
        function save_vec_varma_forecast_summary(self, path)
        ...
        function save_vec_varma_conditional_forecast_summary(self, path)
        ...
        function save_vec_varma_irf_summary(self, path)
        ...
        function save_vec_varma_fevd_summary(self, path)
        ...
        function save_vec_varma_hd_summary(self, path)
